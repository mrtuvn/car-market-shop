...
const ProductFlashSellCard: React.FC<Props> = ({ product, className, date }) => {
  ...
  const RenderAddToCart: React.FC<Props> = ({ product }) => {
    ...
    if (availabilityStatus === 'Low Stock') {
      return (
        <Link
          className="block items-center justify-center rounded-full bg-primary px-4 py-2 text-sm font-medium leading-6 text-white focus:outline-none focus-visible:outline-none"
