...
const ProductFlashSellCard: React.FC<Props> = ({ product }) => {
  const {
    id,
    title,
    category,
    ...
    if (availabilityStatus === 'Low Stock') {
      return (
        <Link
          ...
          href={`${ROUTES.CATEGORIES}/${category}/${slug}-${id}`}
        >
          Product Details
        </Link>
      )
  ...
  return (
    <div className="group gap-2 rounded border bg-white p-2">
      <div className="relative overflow-hidden">
        ...
        <button
          className="shadow-quickview absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 scale-0 transform rounded-full bg-white px-4 py-4 opacity-0 transition group-hover:scale-100 group-hover:opacity-100"
