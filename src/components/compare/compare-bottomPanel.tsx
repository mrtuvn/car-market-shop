...
import { clearCompare, removeFromCompare } from '../../slices/compare/compareSlice'
        ...
        <div className="fixed bottom-0 z-40 w-full bg-black/75 backdrop-blur-sm">
          <Container>
            <button
              ...
              aria-expanded="true"
            >
              <BsChevronDown className={cn('h-6 w-6 text-white', compareToggle && 'rotate-180')} />
              <span className="sr-only">Compare</span>
            </button>
            <div className="flex justify-between py-3 text-sm text-white">
              <div className="c-text-contents flex items-center">Compare({compareList.length})</div>
            ...
            {compareToggle && (
              <div className="mb-4 grid grid-cols-4 gap-5">
                ...
                {Array.from({ length: compareOtherItem }).map((_, id) => (
                  <div key={id} className="group rounded-lg bg-orange-50 p-2"></div>
                ))}
              </div>
            )}
          </Container>
