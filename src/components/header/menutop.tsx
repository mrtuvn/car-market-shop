...
const HeaderMenutop: React.FC<MenuProps> = ({ className }) => {
  ...
  return (
    <div className={cn('smx-auto pace-s-5 flex flex-shrink-0', className)}>
      <nav className="relative flex items-center transition-all duration-200 ease-in-out">
        <ThemeToggle />
        <div className="menuItem group mx-2 cursor-pointer md:mx-3">
          <Link to="/wishlist" className="flex items-center gap-1 py-2 hover:text-blue-500">
            <IoIosHeart className="h-4 w-4" />
            Wishlist({wishlistList.length})
          </Link>
        </div>
        <div className="menuItem group mx-2 cursor-pointer md:mx-3">
          <Link to="/compare" className="flex items-center gap-1 py-2 hover:text-blue-500">
