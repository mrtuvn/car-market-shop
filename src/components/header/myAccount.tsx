...
export const MyAccount: React.FC<Props> = ({ className }) => {
  ...
  if (isLoggedIn)
    return (
      <div className="accountButton shrink-0 items-center lg:flex">
        <Menu>
          <MenuButton as="div" className="flex items-center justify-center gap-1">
          ...
          >
            {links.map((link) => (
              <MenuItem key={link.href}>
                <Link className="block truncate py-1 leading-6 hover:text-blue-500" to={link.href}>
                  {link.label}
                </Link>
              </MenuItem>
            ))}
  ...
  return (
    <Link to={'/login'} className="accountButton shrink-0 items-center lg:flex">
      ...
      <button className="ms-2 text-sm font-normal focus:outline-none" aria-label="Authentication">
        My Account
      </button>
    </Link>
  )
